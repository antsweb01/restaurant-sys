        ...
        <div class="card">
            <div class="card-body">
                <legend class="font-weight-semibold text-uppercase font-size-sm">
                    <i class="icon-address-book mr-2"></i> Kitchen Code: ({{ $user->kitchen->code }}) | Kitchen's Stores
                </legend>
                <div class="form-group row form-group-feedback form-group-feedback-right">
                    @if(count($userRestaurants) === 0)
                    <div class="col-lg-9">
                        <p class="text-warning">{{ $user->kitchen->name }} has no restaurant to assigned yet.</p>
                    </div>
                    @else
                    <br>
                    <div class="col-lg-9">
                        <p><strong>{{ $user->name }}</strong> is serving <strong>{{ $userRestaurants->count() }} </strong> stores.</p>
    ...
    <div class="col-md-12 hidden" id="manageRestaurantsBlock">
        <div class="form-group form-group-feedback form-group-feedback-right search-box">
            <input type="text" class="form-control form-control-lg search-input"
                placeholder="Filter with store name...">
        ...
        <div class="card">
            <div class="card-body">
                <div class="col-lg-12">
                    <form action="{{ route('admin.updateKitchenAdminKitchen') }}" method="POST">
