    ...
    <div class="row">
        <div class="col-lg-8 mt-4">
            ...
            <button type="button" class="btn btn-sm btn-secondary my-2 ml-2 thermalPrintButton" disabled="disabled"
                title="{{__('thermalPrinterLang.connectingToPrinterMessage')}}"
            ...
            <a href="javascript:void(0)" id="printButton" class="btn btn-sm btn-primary mt-2"
                style="color: #fff; border: 1px solid #ccc; float: right; border-radius: 8px"><i
                    class="icon-printer mr-1"></i>{{__('storeDashboard.ovPrint')}}</a>
            ...
            <div class="sidebar-category mt-1"
                ...
                <div class="category-content" id="printThis">
                    <div href="#" class="btn btn-block content-group"
                        ...
                        <strong style="font-size: 1.2rem;">{{ $order->unique_order_id }}</strong>
                    </div>
                    <div class="p-3">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="form-group mb-0">
                                <label
                                    class="control-label no-margin text-semibold mr-1"><strong>{{__('storeDashboard.ovOrderPlaced')}}:</strong></label>
                                ...
                                <div class="col-md-6 col-sm-12">
                                    <div class="form-group">
                                        <label class="control-label no-margin text-semibold mr-1">
                                            <strong>
                                                <h5 class="font-weight-bold">{{__('storeDashboard.ovCustomerDetails')}}
                                                </h5>
                                            </strong>
                                        </label>
                                        <br>
                                        <p><b>{{__('storeDashboard.ovName')}}: </b> {{ $order->user->name }}</p>
                                        @if(config('setting.hideCustomerDetailsFromStoreOwner') == "false")
                                        <p><b>{{__('storeDashboard.ovEmail')}}: </b> {{ $order->user->email }}</p>
                                        <p><b>{{__('storeDashboard.ovContactNumber')}}: </b> {{ $order->user->phone }}
                                        </p>
                                        ...
                                        <p><b>{{__('storeDashboard.ovTaxNumber')}}: </b>
                                            {{ strtoupper($order->user->tax_number) }}</p>
                                        @endif
                                        @if($order->order_comment != NULL)
                                        <p class="mb-0"><b>{{__('storeDashboard.ovCommentSuggestion')}}:</b></p>
                                    ...
                                    <div class="form-group mb-1">
                                        <div class="d-flex justify-content-center align-items-center flex-column mb-1"
                                            ...
                                            <div class="py-1" style="font-weight: 900;">
                                                {{__('storeDashboard.ovStatusText')}}</div>
                                            <hr style="width: 100%;" class="m-0">
                                            <div class="py-1 text-success @if ($order->orderstatus_id == 6) text-danger @endif"
                                                style="font-weight: 500;">
                                                @if ($order->orderstatus_id == 1)
                                                {{__('storeDashboard.opOrderStatus1')}} @endif
                                                @if ($order->orderstatus_id == 2)
                                                {{__('storeDashboard.opOrderStatus2')}} @endif
                                                @if ($order->orderstatus_id == 3)
                                                {{__('storeDashboard.opOrderStatus3')}} @endif
                                                @if ($order->orderstatus_id == 4)
                                                {{__('storeDashboard.opOrderStatus4')}} @endif
                                                @if ($order->orderstatus_id == 5)
                                                {{__('storeDashboard.opOrderStatus5')}} @endif
                                                @if ($order->orderstatus_id == 6)
                                                {{__('storeDashboard.opOrderStatus6')}} @endif
                                                @if ($order->orderstatus_id == 7)
                                                {{__('storeDashboard.opOrderStatus7')}} @endif
                                                @if ($order->orderstatus_id == 10)
                                                {{__('storeDashboard.opOrderStatus10')}} @endif
                                                @if ($order->orderstatus_id == 11)
                                                {{__('storeDashboard.opOrderStatus11')}} @endif
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="d-flex">
                                            <div class="col p-0 mr-2">
                                                <div class="d-flex justify-content-center align-items-center flex-column mb-1"
                                                    style="border: 1px solid #ddd;">
                                                    <div class="py-1" style="font-weight: 900;">
                                                        {{__('storeDashboard.ovOrderType')}}</div>
                                                    <hr style="width: 100%;" class="m-0">
                                            ...
                                            <div class="col p-0">
                                                <div class="d-flex justify-content-center align-items-center flex-column mb-1"
                                                    style="border: 1px solid #ddd;">
                                                    <div class="py-1" style="font-weight: 900;">
                                                        {{__('storeDashboard.ovPaymentMode')}}</div>
                                                    <hr style="width: 100%;" class="m-0">
                                                    <div class="py-1 text-warning" style="font-weight: 500;">
                                                        {{ $order->payment_mode }}
                                                    </div>
                                    ...
                                    <div class="col-md-12 p-2 mb-3"
                                        ...
                                        <div>
                                            <div class="d-flex mb-1 align-items-start" style="font-size: 1.2rem;">
                                                <span
                                                    class="badge badge-flat border-grey-800 text-default mr-2">x{{ $item->quantity }}</span>
                            ...
                            <table class="table table-bordered table-striped bill-calc-table">
                                <tr>
                                    ...
                                    <td class="text-right td-data">
                                        {{ config('setting.currencyFormat') }}{{ $subTotal }}</td>
                                </tr>
                                @if($order->coupon_name != NULL)
                                <tr>
                                    <td class="text-left td-title">{{__('storeDashboard.ovCoupon')}}</td>
                                    <td class="text-right td-data"> {{ $order->coupon_name }} @if($order->coupon_amount
                                        != NULL) ({{ config('setting.currencyFormat') }}{{ $order->coupon_amount }})
                                        @endif</td>
                                </tr>
                                @endif
                                ...
                                <tr>
                                    ...
                                    <td class="text-right td-data">{{ $order->tax }}% @if($order->tax_amount != NULL)
                                        ({{ config('setting.currencyFormat') }}{{ $order->tax_amount }}) @endif</td>
                                </tr>
                                @endif
                                @if($order->wallet_amount != NULL)
                                <tr>
                                    <td class="text-left td-title">{{__('storeDashboard.ovPaidWithWallet')}}
                                        {{ config('setting.walletName') }}</td>
                                    <td class="text-right td-data">
                                ...
                                <tr>
                                    ...
                                    <td class="text-right td-data"> {{ config('setting.currencyFormat') }}{{ $total }}
                                    </td>
                                </tr>
                                @php
                                if(!is_null($order->tip_amount)) {
                                ...
                                <tr>
                                    ...
                                    <td class="text-right td-data"><b>
                                            {{ config('setting.currencyFormat') }}{{ $payable }}</b></td>
                                </tr>
                                @endif
                            </table>
                        </div>
        ...
        <div class="col-md-4 mb-5">
            ...
            <div class="card">
                <div class="card-body">
                    <p class="text-center mb-0">
                        <b>
                            {{ __('orderScheduleLang.orderScheduleTitleText') }}
                        </b>
                        <br>
                        <b>{{ __('orderScheduleLang.orderScheduleDateText') }}</b>
                        {{ json_decode($order->schedule_date)->day }}, {{ json_decode($order->schedule_date)->date }}
                        <br>
                        <b>{{ __('orderScheduleLang.orderScheduleSlotText') }}:</b>
                        {{ json_decode($order->schedule_slot)->open }} - {{ json_decode($order->schedule_slot)->close }}
                    </p>
                </div>
            </div>
            @endif
            ...
            <div class="card">
                <div class="card-body">
                    <p class="text-center mb-0"> <strong>{{ __('storeDashboard.opDeliveryBy') }}:
                            {{ $order->accept_delivery->user->name }}</strong></p>
                </div>
            </div>
            @endif
            @endif
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-center">
                        <a href="{{ route('restaurant.confirmScheduledOrder', $order->id) }}"
                            class="mr-2 btn btn-lg confirmOrderBtn btn-success">
                            {{__('orderScheduleLang.dashboardConfirmScheduledOrder')}} <i
