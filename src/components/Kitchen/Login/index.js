...
class Login extends Component {
	...
	handleLogin = (event) => {
		...
		this.props.loginKitchenUser(this.state.email, this.state.password);
	};

	componentWillReceiveProps(nextProps) {
		const { kitchen_user } = this.props;
		if (kitchen_user !== nextProps.kitchen_user) {
			this.setState({ loading: false });
			if (nextProps.kitchen_user.success === false) {
				this.setState({ error: true });
			}
		}

		if (nextProps.kitchen_user.success) {
			if (navigator.userAgent === "FoodomaaAndroidWebViewUA") {
				if (window.Android !== "undefined") {
					window.Android.registerFcm(
						nextProps.kitchen_user.data.auth_token,
						nextProps.kitchen_user.data.id
					);
				}
			}
		}
					...
					<div className="row">
						<div className="col-12 col-sm-6 col-lg-4 mx-auto">
							<div className="text-center clipped-delivery-login-header">
								...
								<Zoom duration={1000}>
									<img
										src="/assets/img/various/cooking.png"
...
export default connect(
	mapStateToProps,
	{ loginKitchenUser, getSettings, getAllLanguages, getSingleLanguageData }
)(Login);
